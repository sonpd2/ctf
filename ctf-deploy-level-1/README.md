...
ctf board | ctf watch (LIVE) | ctf score
```
...
## Terminal & cách ly
Linux đầy đủ, không giới hạn lệnh, có mạng tới backend (cho `ctf`). Mỗi phiên = **sandbox bwrap riêng**: PID/IPC tách biệt, home tạm reset khi thoát, hệ thống bị xâm nhập overlay **chỉ đọc**. Container giới hạn pids/mem/cpu, cap_drop, no-new-privileges.
